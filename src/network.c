...
void forward_network(network *netp)
{
    ...
    network net = *netp;
    //what is net.inputs and net.outputs
    //mine: here net should have X in net.input and y in net.truth
    image im = float_to_image(net.w, net.h, 3, net.input);
    show_image(im, "forward_network", 1); // the third parameter is millsecond
    cvWaitKey(0);
    
    box b = float_to_box(net.truth, 1);
    printf("------mine: net.truth : %f %f %f %f\n", b.x, b.y, b.w, b.h);
    /*
    int k;
    for(k = 0; k < l.max_boxes; ++k){ //l.max_boxes is 90 for max object slots
       ...
       if(!b.x) break;
       printf("------mine: net.truth : loaded: %f %f %f %f\n", b.x, b.y, b.w, b.h);
    }
    */
    //----------
    ...
    for(i = 0; i < net.n; ++i){
        ...
        //layer l = net.layers[i]; //orig
        //---mine:
        l = net.layers[i];
        printf("---mine: network.c: forward_network\n");        
        printf("---mine: net layer level: %d \n",i);        
        printf("---mine: net layer type: %s \n",get_layer_string(l.type));
        printf("---mine: net.n is total number of layers of net: %d \n",net.n);        
        printf("---mine: layer.n is number of filters: %d \n",l.n);        
        printf("\n");        
        printf("---mine: l.out_c: %d\n",l.out_c);
        printf("\n");        
        printf("\n");        
        //image im=get_convolutional_image(l);
        //image im=float_to_image(l.in_w,l.in_h,l.in_c,l.input); // layer has no l.input memeber
        //image im=float_to_image(l.out_w,l.out_h,l.out_c,l.output);
        image im=float_to_image(l.out_w,l.out_h,3,l.output); //nothing 
        show_image(im, "forward_network layer before forward", 1);	
        cvWaitKey(0);
        //if l.type == "YOLO"
        //----------
        ...
        l.forward(l, net);
        //mine
        im=float_to_image(l.out_w,l.out_h,3,l.output); //nothing 
        show_image(im, "forward_network layer after forward", 1);
        cvWaitKey(0);        
        //-----------
    ...
    //??printf("---mine: here already get the Y in this batch run\n");
...
float train_network_datum(network *net)
{
    *net->seen += net->batch;
    ...
    //mine: here *net should have X in net.input and y in net.truth
    image im = float_to_image(net->w, net->h, 3, net->input);
    show_image(im, "train_network_datum", 100); // the third parameter is millsecond
    cvWaitKey(0);
    //----------
    forward_network(net);
...
float train_network(network *net, data d)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        //mine: here *net should have X in net.input and y in net.truth
        image im = float_to_image(net->w, net->h, 3, net->input);
        show_image(im, "train_network", 1); // the third parameter is millsecond
        cvWaitKey(0);
        //----------
        float err = train_network_datum(net);
        sum += err;
